...
Ebben a fejezetben az \href{http://openflights.org/}{openflights.org} oldalon elérhető repülési adatbázis feldolgozását és ezen adatokon futtatott szimulációt mutatom be.
  ...
  Ebben a részben elsőként a referenciaként szolgáló szimulációt mutatom be, majd bemutatom minden algebra szimulációs eredményét, a fejezet végén pedig egy táblázatban foglalom össze az eredményeket.

        \iffalse
        %----------------------------------------------------------------------------
        \subsection{A referencia szimuláció}
        %----------------------------------------------------------------------------
        A referencia szimuláció célja az, hogy viszonyítási alapot adjon a pontrendszernek. Ebben az esetben
        Értelemszerűen a pontpárok közötti mutatókat nem kell futtatni, hiszen
        \Histo{referencia}{A referencia szimuláció során generált gráf fokszámeloszlása.}{fig:refer}
        \AvgPlot{referencia}{A referencia szimuláció során generált gráf átlagfokszám-eloszlása.}{fig:refer-avg}
        \fi
  ...
  Látható, hogy a csomópontok ki- és befoka minden pontra közel megegyezik. Az átlagfokszám-eloszlás \aref{fig:S-avg}. ábrán látható. Hasonlóan a másik diagramhoz, itt is egy csoportosított ((a) diagram) és egy kinagyított ((b) diagram) látható. Az ábráról leolvasható, hogy a generált gráf skálafüggetlen, így $DD$ = 1.
  \AvgPlot{shortest}{A $\mathcal{S}$ algebra által generált gráf csomópontonkénti átlagfokszám-eloszlása.}{fig:S-avg}

  A $GD$ (átmérő) mutató 3, mert a legmagasabb lépésszámú út 3 lépést tartalmaz. Az élösszefüggőségi mutatót a felső 20\%-ból számítva $C$ = 16. Az $EC$ (élszám) pontértéke 9,5872 ($ln(14578)$). Így a Shortest algebrára a $$GV = \frac{DD \cdot GD \cdot C}{ln(EC)} = \frac{1 \cdot 3 \cdot 16}{9,5872} \approx 5,006.$$.
